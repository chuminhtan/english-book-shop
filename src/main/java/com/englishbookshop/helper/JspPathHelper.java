package com.englishbookshop.helper;

public class JspPathHelper {
	public static final String INDEX = "frontend/index.jsp";
	public static final String ADMIN_LOGIN = "admin-login.jsp";
	public static final String ADMIN_HOME = "dashboard.jsp";
	
	public static final String DASHBOARD = "dashboard.jsp";
	
	public static final String USER_LIST = "user-list.jsp";
	public static final String USER_CREATE = "user-form-create.jsp";
	public static final String USER_EDIT = "user-form-edit.jsp";
	
	public static final String CATEGORY_LIST = "category-list.jsp";
	public static final String CATEGORY_CREATE = "category-form-create.jsp";
	public static final String CATEGORY_EDIT = "category-form-edit.jsp";
	
	public static final String BOOK_LIST = "book-list.jsp";
	public static final String BOOK_CREATE = "book-form-create.jsp";
	public static final String BOOK_EDIT_FORM = "book-form-edit.jsp";
	public static final String BOOKS_LIST_BY_CATEGORY = "frontend/books-list-by-category.jsp";
	public static final String BOOK_DETAIL = "frontend/book-detail.jsp";
	public static final String BOOK_SEARCH = "frontend/book-search.jsp";
	
	public static final String CUSTOMER_LIST = "customer-list.jsp";
	public static final String CUSTOMER_CREATE = "customer-form-create.jsp";
	public static final String CUSTOMER_EDIT = "customer-form-edit.jsp";
	public static final String CUSTOMER_REGISTER = "frontend/customer-form-register.jsp";
	public static final String CUSTOMER_REGISTER_SUCCESS = "frontend/customer-register-success.jsp";
	public static final String CUSTOMER_PROFILE = "../frontend/customer/customer-profile.jsp";

	public static final String REVIEW_LIST = "review-list.jsp";
	public static final String REVIEW_EDIT = "review-form-edit.jsp";
	
	public static final String SHOPPING_CART = "frontend/shopping-cart.jsp";
	
	public static final String ORDER_LIST = "order-list.jsp";
	public static final String ORDER_DETAIL = "order-detail.jsp";
	public static final String ORDER_EDIT= "order-edit.jsp";
	public static final String ORDER_CHECKOUT = "frontend/checkout.jsp";
	public static final String ORDER_CHECKOUT_MESSAGE = "frontend/checkout-message.jsp";
	public static final String ORDER_SUCCESS = "frontend/place-order-success.jsp";
	public static final String ORDER_VIEW = "../frontend/customer/view-orders.jsp";
	public static final String ORDER_VIEW_DETAILS = "../frontend/customer/view-order-details.jsp";

}
